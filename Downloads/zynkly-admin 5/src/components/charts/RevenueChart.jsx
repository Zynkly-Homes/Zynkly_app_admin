...
function processRevenueData(rawData, days = 30) {
  ...
  rawData.forEach((booking) => {
    if (!booking.work_completed_at) return;
    try {
      const day = format(parseISO(booking.work_completed_at), 'yyyy-MM-dd');
      // Use total_amount (DB column) or total (legacy/fallback)
      const amount = booking.total_amount ?? 0;
      byDate[day] = (byDate[day] || 0) + amount;
    } catch {
      console.warn('[RevenueChart] Invalid date in booking:', booking.work_completed_at);
    }
  });
