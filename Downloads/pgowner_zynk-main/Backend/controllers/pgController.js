...
exports.createRequest = async (req, res) => {
    const { roomNumber, date, notes, pgName, pgContact } = req.body;
    try {
...
exports.getMyRequests = async (req, res) => {
    try {
        const requests = await CleaningRequest.find({ pgId: new mongoose.Types.ObjectId(req.user.id) }).sort({ date: -1 });
        res.json(requests);
    } catch (err) {
        res.status(500).json({ message: 'Server Error' });
    }
